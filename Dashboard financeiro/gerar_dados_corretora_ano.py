"""Gera um ano inteiro de dados da CORRETORA para o filtro de Ano do painel.

Companheiro de ``atualizar_dados.py`` e ``gerar_drilldown_dre.py``, mas com um
propósito bem específico: o painel exclusivo da CORRETORA
(``painel_financeiro_corretora.html``) tem um seletor de Ano (2025/2026, por
ora), e cada ano precisa do mesmo conjunto de blocos que o painel principal —
só que **de uma empresa só**, para não pesar o arquivo.
...
consultas, só que com o intervalo de datas do ano pedido.
...
Uso (a partir da raiz do projeto)::
    ...
    .venv\\Scripts\\python.exe "Dashboard financeiro/gerar_dados_corretora_ano.py" --ano 2025 > ano_2025.js
"""
...
try:
    ...
    raise SystemExit(
        ...
        '  .venv\\Scripts\\python.exe "Dashboard financeiro/gerar_dados_corretora_ano.py"'
    ) from _exc
...
def _filtro_empresa_url(query: str) -> str:
    """``_q_*`` de atualizar_dados.py não filtra empresa — filtramos no pós-processo
    (mantém a mesma consulta testada; simplesmente ignoramos as outras linhas)."""
    return query
...
def _emit_flat_dict(nome: str, d: dict[str, list[float]], indent: str) -> list[str]:
    linhas = [f"{indent}{nome}: {{"]
    itens = [f"{k}:{_lista(v)}" for k, v in d.items()]
    for i, it in enumerate(itens):
        linhas.append(f"{indent}  {it}{',' if i < len(itens)-1 else ''}")
    linhas.append(f"{indent}}},")
    return linhas


def _emit_nested_dict(nome: str, d: dict[str, dict[str, list[float]]], indent: str) -> list[str]:
    linhas = [f"{indent}{nome}: {{"]
    chaves = sorted(d)
    for i, chave in enumerate(chaves):
        linhas.append(f"{indent}  {chave}: {{")
        descs = sorted(d[chave])
        for j, desc in enumerate(descs):
            v = "," if j < len(descs) - 1 else ""
            linhas.append(f"{indent}    '{desc}':{_lista(d[chave][desc])}{v}")
        linhas.append(f"{indent}  }}{',' if i < len(chaves)-1 else ''}")
    linhas.append(f"{indent}}},")
    return linhas
